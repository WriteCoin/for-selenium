...
import winsound
from captcha import manual_solve_captcha, solve_captcha
from element import By, FindOptions, switch_through_default
import undetected_chromedriver as uc
from selenium.webdriver.remote.webelement import WebElement
from selenium.webdriver.support.ui import WebDriverWait

from script import new_chrome_script, threading_start
...
def recaptcha_pre_solve(driver: uc.Chrome, checkboxOp: FindOptions = defaultCheckboxOp) -> None:
...
defaultTaskFrameOp = FindOptions(By.CSS_SELECTOR, 'iframe[title="текущую проверку reCAPTCHA можно пройти в течение ещё двух минут"]')
...
def solve_recaptcha(
  ...
  def solve_fun(driver: uc.Chrome):
    switch_through_default(driver, lambda: driver.find_element(taskFrameOp.by.value, taskFrameOp.value))
    solveFun(driver)
    switch_through_default(driver, lambda: driver.find_element(frameOp.by.value, frameOp.value))
  return solve_captcha(driver, solve_fun, condSolve, frameOp, preSolveFun, maxSolveTime, singleCall, frequency)
