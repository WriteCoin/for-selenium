...
BrowserOptions: TypeAlias = dict[str, str]
...
class Script(ABC):
  ...
  def run(self, 
      ...
      driver_options: uc.ChromeOptions = None,
      printError: Callable[[Exception], None] = default_print_error,
      viewTrace: bool = True  
    ):
    try:
      driver_options = self.default_driver_options() if driver_options is None else driver_options
  ...
  def run_with_handle_errors(self, driver: uc.Chrome = None, driver_options: uc.ChromeOptions = None):
    try:
